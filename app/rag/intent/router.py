...
router = APIRouter(prefix="/intent-tree", tags=["intent"], dependencies=[Depends(require_user)])
...
async def trees(request: Request) -> dict:
    return Results.success([_vo(node) for node in await _service(request).list_tree()]).model_dump(by_alias=True)


@router.post("")
async def create(body: IntentNodeWrite, request: Request, user: Annotated[LoginUser, Depends(require_user)]) -> dict:
    try:
...
@router.put("/{node_id}")
async def update(node_id: int, body: IntentNodeWrite, request: Request, user: Annotated[LoginUser, Depends(require_user)]) -> dict:
    try:
