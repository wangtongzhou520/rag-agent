...
容器部署不只做健康检查：在容器栈上开启 `RAGENT_EVAL_ENABLED=true` 并注入百炼密钥后，
用 `scripts/seed_eval_corpus.py` 播种评测语料，再对容器 API 跑完整质量门禁，结果如下。

| 套件 | 规模 | 关键指标 | 结果 |
|---|---|---|---|
| 事实问答检索（默认） | 42 题 | Hit Rate 1.0、MRR 1.0、Context Precision 0.9524、Latency P95 1059ms | 通过 |
| 无答案安全集（含答案 + 语义裁判） | 8 题 | 拒答率 1.0、Semantic Pass 1.0、无证据率 0.25（仅观察） | 通过 |
| 事实问答答案冒烟（`--with-answers --limit 8`） | 8 题 | 关键事实覆盖 1.0、完整答案率 1.0、答案 P95 1780ms | 通过 |

播种脚本的冷启动与幂等两条路径都在该容器栈上验证过：全新集合 `m5_quality_baseline_seedcheck`
创建知识库、上传 3 篇语料、触发 3 次分块并等待至 `seeded=true`；对已完成的
`m5_quality_baseline` 重跑则为零上传、零重建。三份报告都通过管理端批次接口发布，可在
评测页按 label 查看。
