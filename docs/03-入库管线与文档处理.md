...
> 基线约束见 `00-总体架构与技术选型.md`：FastAPI + SQLAlchemy 2.0 async + pgvector + redis-py + httpx，不引入 LangChain 系框架与外部消息 broker（ARQ/Celery/RocketMQ 均不使用，异步任务走自研 PG 队列）；全新项目由启动期 Schema 初始化建表；REST 路径挂在 `root_path=/api/ragent` 下，保持现有前端所需的 `Result<T>` 契约。
...
- [x] `ChunkBudget`：默认 `(maxChars=1024, overlapChars=128, rowsPerChunk=50, toleranceFactor=3)`，上限与校验规则对齐
...
- [x] `ChunkPacker` 语义：相邻**同节同来源**块按 `maxChars` 预算打包（碎块自动合并），章节或 sheet 变化即断开；超预算块下沉到 `split_text`
- [x] `TextSplitter`：滑窗 + 句界回退（`\n` → `。！？` → `.!?` + 空白）+ overlap 回退；URL 断行修复与 CJK 软换行合并未实现
- [ ] `ChunkAssembler`：UUIDv7 分配 chunkId、序号分配、向量文本前缀（章节路径去重拼接）、`restore` 重建路径
- [x] `ChunkingService`：两分支（整文档哨兵 / block-aware）
...
- [x] Pipeline CRUD + 节点整体替换（先物理删再插）
- [x] `IngestionEngine`：环校验、单起点校验、链式执行、条件跳过、逐节点 `NodeLog`、失败即中断
- [ ] 条件 DSL：`all/any/not` 组合 + 12 个操作符的 field 规则
- [x] 六类节点：fetcher / parser / enhancer / chunker / enricher / indexer
- [ ] fetcher：本地上传（预置字节短路）、飞书 docx 在线文档、HTTP URL（凭证转 header）
- [x] 任务同步执行 + `t_ingestion_task` / `t_ingestion_task_node` 落库（节点输出摘要 1MB 截断）
...
- [x] 上传并发信号量：`rag.upload.max_concurrency` + `wait_timeout_seconds`，占满即 429 降级（`UploadLimiter`）
- [ ] MinerU 并发信号量（MinerU 解析器本身尚未实现）
...
本章表结构属于项目独立 Schema，由启动期 Schema 初始化创建。仅在关系库内部使用的主键采用 `BIGINT GENERATED BY DEFAULT AS IDENTITY`；需落库前预分配或跨存储对齐的业务标识（chunkId 等）采用 UUIDv7，并在 PostgreSQL 中优先存为原生 `UUID`。统一保留审计列 `create_time/update_time` 和逻辑删 `deleted SMALLINT DEFAULT 0`；`t_async_task` 与业务表一起初始化。
...
- `t_async_task`：见 11.2，与业务表一同在启动期初始化。
...
| GET | `/ingestion/async-tasks` | `current/size/status/taskType`（只读） | `IPage<AsyncTaskVO>` |
...
管理端任务中心额外只读暴露 `AsyncTaskVO{id,eventId,taskType,bizKey,status,retryCount,maxRetries,nextRetryAt,leaseUntil,errorMessage,createTime,updateTime}`。该接口不返回 `payload` 和 `owner`，避免业务内容或 worker 实例信息进入管理页面；所有时间字段统一为 Unix epoch milliseconds。
