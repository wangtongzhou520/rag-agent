...
| SSE 解析 | `src/hooks/useStreamResponse.ts` | 复用分帧器，按基础六事件 + 可选 guidance 契约补完状态机 |
...
| `/login` | 登录 | 公开 | READY |
| `/chat` | 新对话 | 登录 | READY |
| `/chat/:conversationId` | 持续对话/历史会话 | 登录 | READY |
...
| `/admin/dashboard` | 概览与质量指标 | admin | READY |
| `/admin/users` | 用户管理 | admin | READY |
| `/admin/audit-logs` | 业务变更审计 | admin | READY |
| `/admin/ingestion` | Pipeline/任务 | admin | READY |
| `/admin/agents` | Agent/Prompt | admin | READY |
| `/admin/mcp` | MCP 工具 | admin | READY |
| `/admin/settings` | 模型与运行时设置 | admin | READY |
...
- 业务页面样式与功能同目录，通过页面入口显式导入并使用域前缀类名；`styles/globals.css` 只保存 Token、reset、应用壳和真正跨功能的共享规则，不继续堆积页面专属 CSS。
...
| Dashboard | `GET /admin/dashboard/overview` | 使用量 KPI 与周期环比 |
| Dashboard | `GET /admin/dashboard/performance` | Trace 延迟与质量指标 |
| Dashboard | `GET /admin/dashboard/trends` | 使用量、耗时和质量趋势 |
| 用户 | `GET/POST /users`、`PUT/DELETE /users/{id}` | 用户分页与 CRUD |
| 用户 | `PUT /user/password` | 当前用户修改本人密码 |
| 审计 | `GET /biz-change-logs`、`GET /biz-change-logs/{id}` | 变更分页与详情 |
...
- `guidance` 保存服务端生成的范围 query；选项或“全部范围”点击后开启下一轮问答。
- 刷新、切路由和重新提问前必须 abort 旧连接，用 generation id 丢弃晚到帧。
- 「停止生成」调用 `POST /rag/v3/stop?taskId=...`，继续读取服务端的 `cancel/done` 终态；
  接口失败时 abort 当前连接作为兜底。M5 已接入 Redis 跨实例取消广播；Redis 异常时仍保留
  单 API 实例取消能力。
...
- 歧义引导同时保留普通文本降级和结构化 guidance 范围面板；历史记录无结构化字段时仍显示文本。
...
**布局**：左侧最近会话栏+中央问答流+右侧来源 Drawer。

- 会话栏按 `lastTime` 倒序展示标题和本地时间，支持历史回放、重命名、删除以及移动端抽屉。
- 新对话收到完整 `done` 后进入 `/chat/:conversationId`；刷新或直接访问该 URL 时从服务端恢复消息。
...
- 顶部包含全局知识搜索入口、返回问答和用户菜单；嵌套管理页在内容区提供上下文路径和返回入口。
- 全局搜索并行使用 `/knowledge-base` + `/knowledge-base/docs/search`，输入防抖 `250ms`；结果按知识库和文档分组，分别进入文档列表和 Chunk 工作台。
- 支持 `⌘K` / `Ctrl+K` 聚焦、方向键选择、`Enter` 打开和 `Esc` 关闭，移动端收敛为三列紧凑工具栏。
...
### 10.12 Dashboard `READY`

- `/admin` 默认进入系统概览；管理员可切换 24 小时、7 天和 30 天统计周期。
- 六项使用量 KPI 严格区分「总量的本周期新增」与「窗口值的前周期环比」，不混用口径。
- 响应表现展示成功率、平均/P95 耗时、错误率、无知识率和 20 秒以上慢请求率；空样本显示后端定义的 0，不虚构数据。
- 趋势支持会话、消息、活跃用户、响应耗时和质量；小时/天粒度由窗口自动选择，缺桶补零。
- 桌面端使用高密度横向指标带，小屏重排为双列；趋势图使用原生 SVG 和明亮蓝色主线，不引入装饰性 AI 图形。
- 页面覆盖 loading/error/empty、手动刷新、周期与指标切换；三个接口和路由均由服务端 `require_admin` 守卫。

### 10.13 用户与审计日志 `READY`

- 用户列表支持用户名/角色搜索、分页、创建、编辑、重置密码和逻辑删除；默认 `admin` 行明确标记且编辑/删除入口禁用，服务端仍执行最终保护。
- 本人改密入口同时出现在问答侧栏和用户页；当前密码校验成功后清理该用户全部 Redis 登录会话，前端回到登录页。
- 创建/编辑表单不回填密码；密码留空表示不修改，角色仅允许管理员/普通用户两种稳定枚举。
- 审计日志支持业务类型、业务 ID、操作类型、操作人、结果和起止时间筛选，筛选条件与页码写入 URL。
- 详情展示操作来源、调用位置、变更前后快照及 JSON Pointer 字段 Diff；密码明文和哈希不进入任何响应或审计快照。
- 两页继续使用高密度表格、克制蓝色交互和清晰的成功/失败状态，小屏筛选器改为单列，宽表在自身容器中横向滚动。

### 10.14 Pipeline 与任务

- Pipeline 列表直接展示从起点沿 `nextNodeId` 的线性执行顺序、节点数和更新时间；搜索词与页码写入 URL。
- 新建和编辑使用顺序节点编辑器，上移/下移会重新生成 `nextNodeId`，节点 ID、类型、参数 JSON 和条件 JSON 均可审阅；保存前检查空 ID、重复 ID 和 JSON 格式，服务端再次校验环、悬空引用、单起点和不可达节点。
- 调试运行支持文件、URL 和飞书三种来源；包含 chunker/indexer 时要求填写真实向量空间。接口同步返回终态，不伪造进度，解析、Embedding、LLM 和 pgvector 的错误落到对应节点。
- 运行列表展示终态、来源、分块数和端到端耗时；详情按执行序展开节点状态、耗时、消息、错误与摘要输出。输出仅保存摘要并限制 1MB，不展示源文件字节、向量或凭证。
- 后台任务视图只读展示 `t_async_task` 的任务类型、业务键、状态、重试次数、下次执行时间和租约，用于检查文档分块等 PG 队列，不与同步 Pipeline 任务混用状态机。
- 页面使用高密度白底/浅蓝控制台布局，不采用自由拖拽画布或装饰性 AI 卡片；390px 下页头、标签和编辑表单重排，宽表在表格容器内滚动。

### 10.15 智能体与 Prompt `READY`

- 页面采用“配置版本 + Prompt 槽位登记册”双栏工作台：左侧选择智能体，右侧按 WorkFlow 专属、Agent 专属、通用分组展示 7 个权威槽位。
- 顶部直接读取部署级 `mode` 与 `effectiveSlotTotal`；每个槽位分别展示当前架构是否执行、未生效原因、当前智能体已配置或默认回落，不把保存状态误写为运行状态。
- 内置「默认助手」仅用于查看和作为两级回落终点，前后端均不提供编辑、删除和槽位保存入口；普通智能体支持创建、编辑、激活、删除。
- Prompt 编辑器支持从内置默认复制、留空恢复默认，并展示必须保留的占位符；最终占位符完整性由服务端校验。
- 写操作完成后刷新 Agent 列表与槽位 Query cache；服务端同时清理 Redis resolved-prompts 缓存，下一次问答按新配置解析。
- 页面保持明亮蓝色、白底和高密度配置表，不使用机器人插画、紫色渐变或装饰性“AI 能力”卡片；页面样式独立位于 `features/agents/AgentPage.css`。
- 390px 下按版本列表、当前版本、槽位分组顺序堆叠；Prompt 编辑器保持可滚动，不产生页面级横向溢出。

### 10.16 MCP 管理与调试 `READY`

- 页面采用“服务连接 + 工具检查器”结构：上方展示配置 Server 及在线状态，下方左侧选择工具、右侧检查描述、运行状态、意图绑定数和 Input Schema。
- Server 地址来源于后端部署配置，只读展示；管理员可启停 Server、重新发现工具、单独启停 Tool。两级开关持久化到 PostgreSQL，页面刷新或服务重启后保持。
- 调试 Dialog 根据 Input Schema 生成 JSON 起始参数，由管理员显式编辑后直接调用，不经过意图识别或 LLM 提参；结果展示成功状态、耗时、文本与结构化内容。
- 页面时间统一按 epoch milliseconds 转本地时间；离线、空工具、协议错误均为业务状态，不伪造成浏览器网络故障。
- 使用明亮蓝色、高密度边框和扁平工作台布局，不使用机器人图标、渐变光晕或装饰性能力卡；全部样式位于 `features/mcp/McpPage.css`。
- 390px 下服务卡、工具列表、详情和调试操作依次单列排列，不产生页面级横向溢出。

### 10.17 运行时与模型设置 `READY`

- 页面定位为只读“运行登记册”，展示当前 API 进程实际加载的执行架构、检索后端、模型路由与熔断状态；不做模型市场或凭据编辑器。
- Chat 档位按 FAST/STANDARD/DEEP 展示候选失败切换顺序和超时预算；模型登记册可切换 Chat、Embedding、Rerank，展示 Provider、默认/档位位置、维度/思考能力和有效状态。
- Provider 连接只展示 URL、端点名称、是否配置和脱敏后的 API Key；前端不得保存、复制或回传完整凭据。
- 检索参数区域展示向量维度、TopK、召回/Rerank 预算、超时、RRF、熔断窗口、记忆轮数和流式分片等当前有效值，并明确修改 YAML/环境变量后需要重启 API。
- 页面保持白底、明亮蓝色和运维登记表密度，候选链是本页识别元素；不使用机器人、模型头像、紫色渐变或营销卡片。样式独立位于 `features/runtime/RuntimeSettingsPage.css`。
- 小屏将摘要重排为双列、档位改为纵向段落；候选链和模型宽表仅在自身容器内横向滚动，不产生页面级溢出。
...
| Conversation/Message 时间 | Unix epoch milliseconds | 使用用户本地时区格式化 |

### 12.2 契约准入

- API 响应不在页面直接 `as SomeType`，关键契约用 Zod parse 或开发环境 assert。
- 所有 ID 在前端默认用 string，不对 BIGINT/UUID 做不必要的 Number 转换。
- 时间按接口契约传输；Trace、Conversation 和 Message 使用 Unix epoch milliseconds，格式化时使用用户本地时区。
...
- SSE 保存七事件黄金样本，回归 event 名、payload camelCase、终态顺序。
...
7. MCP Server 在线 → 检查发现工具 → 打开显式参数调试 → 查看结构化结果。
...
F2 第七批于 2026-09-03 完成：控制台顶部全局搜索并行检索知识库和文档，使用 `250ms`
防抖并按领域分组；知识库结果进入文档列表，文档结果进入 Chunk 工作台。交互支持鼠标、
方向键、`Enter`、`Esc` 以及 Apple Silicon/macOS 的 `⌘K`（其他平台为 `Ctrl+K`）；
Playwright 覆盖结果展示、键盘选择、快捷键聚焦和两类跳转。至此 F2 当前规划的管理面全部完成。

### F3 会话产品化（先补后端）

- [x] 会话列表、消息历史、重命名、删除
- [x] 服务端停止任务（F3 单实例；M5 Redis 跨实例广播已完成）
- [x] 消息反馈和推荐问题
- [x] 结构化歧义选项事件

F3 第一批于 2026-09-03 完成：新增 `/conversations` 列表、重命名、删除和消息历史接口，
会话与消息按当前登录用户强制隔离；列表和消息时间统一输出 Unix epoch milliseconds，
assistant 消息回填当前用户 vote。删除在同一事务内软删会话、消息和摘要，删除后的旧会话
禁止继续写入。前端左栏接入最近会话、历史回放、重命名和删除，新会话完成后稳定到
`/chat/:conversationId`，支持刷新恢复；Docker 隔离数据库测试和 Playwright 黄金路径均已覆盖。

F3 第二批于 2026-09-03 完成：`meta.taskId` 与服务端 registry 使用同一任务标识，
`POST /rag/v3/stop` 按当前用户隔离并幂等停止；模型 httpx 流和读取任务同步取消，已生成内容
以 `INTERRUPTED` 落库并发送 `cancel → done`。前端等待协议终态后结束展示，停止接口失败时
退回本地 abort。M5 已增加 Redis 任务归属、取消标记与广播，支持停止请求命中任一 API 实例。

F3 第三批于 2026-09-03 完成：回答底部增加克制的赞同/不赞同二态工具栏，相同选择可撤销，
前端乐观更新且失败回滚；反馈写入复用 PG 任务队列，通过 active 事件版本替换、worker 快照
校验和数据库 `submittedAt` 乱序保护保证最后一次操作获胜。推荐问题由用户主动触发 FAST 档，
最多展示 3 条，支持消息级成功/空结果缓存和失败重试；点击建议可直接继续提问。grounding chunks
随 assistant 消息保存，推荐生成节点写入该会话最近一次 RAG Trace。

F3 第四批于 2026-09-03 完成：新增可选 `guidance` SSE 事件，payload 提供 2–6 个知识范围、
稳定 intentCode 和服务端生成的下一轮 query，并保留普通 message 编号文本降级。前端以紧凑的
双列范围面板展示选项，小屏切换单列，支持选择单一范围或检索全部范围；点击后自动进入下一轮，
状态机、Zod 协议、后端管线和 Playwright 均覆盖 `message → guidance → finish → done`。
至此 F3 会话产品化规划全部完成。
...
F4 第一批于 2026-09-04 完成：新增管理员专用的 `overview/performance/trends` 三个实时
统计接口，窗口解析、前周期环比、P95、成功/错误/无知识/慢请求率和小时/天补零趋势均按
`07` 文档口径实现。控制台默认入口切换为系统概览，提供 24 小时、7 天、30 天周期和
五类趋势切换；页面只呈现数据库真实结果，采用高密度蓝色工作台布局。PostgreSQL 隔离
聚合测试、Vitest 格式化测试及 Playwright 桌面/移动截图均已覆盖。

F4 第二批于 2026-09-04 完成：用户管理 API 与页面支持分页搜索、创建、编辑、角色调整、
密码重置和逻辑删除，默认管理员受到前后端双重保护；本人改密在问答端和控制台均可进入。
身份或密码变化在业务提交前批量清理 Redis 登录会话。新增 `t_biz_change_log`、确定性的
JSON Pointer Diff、独立事务审计记录器及分页/详情接口，用户写操作成功和失败均进入台账，
密码只记录变更标识。Playwright 覆盖账号创建、角色调整、筛选审计和快照详情，并完成
桌面及 390px 页面复核。

F4 第三批于 2026-09-04 完成：新增 Pipeline、节点、同步任务和节点日志四表，提供 Pipeline
CRUD、文件/URL/飞书同步调试、任务分页、逐节点详情及 PG 后台任务只读接口。执行引擎实现单链
校验、条件跳过、失败中断和六类节点，节点输出按 1MB 截断，所有时间字段输出 epoch
milliseconds；Pipeline 写操作和调试运行进入审计台账。控制台新增配置、调试运行、后台任务
三视图，Playwright 覆盖创建、线性节点展示、运行详情以及桌面/390px 视觉复核。

F4 第四批于 2026-09-05 完成：新增 Agent Profile 与 7 项 Prompt Slot 持久化、内置默认
助手种子、全局单激活、两级 Prompt 回落和 Redis 一小时缓存；闲聊、知识库、MCP 与推荐
问题四条现有运行路径已读取解析后的配置。管理页支持人设 CRUD、激活、按架构分组查看、
未生效原因、默认复制、留空回落与占位符提示，样式独立按 feature 拆分。后端 157 项单测、
11 项 Docker 隔离集成测试、前端 34 项单测、生产构建、Lint 与 Playwright 桌面/移动验收通过。

F4 第五批于 2026-09-07 完成：MCP client 在 FastAPI lifespan 内连接独立 FastMCP 服务并
发现 `weather_query`、`sales_query`、`ticket_query`，使用限定 tool key 注册；连接失败不会
阻断 API，管理员可重新发现。问答链路接入标准档 LLM 提参、默认值/必填/类型/枚举校验、
全局与 per-server 双层限流和安全失败收敛。管理页支持 Server/Tool 持久化启停、Input Schema
检查及显式 JSON 调试，写操作进入审计；页面样式独立位于 MCP feature 目录，桌面与 390px
浏览器验收均无横向溢出。
当前自动化结果：后端 164 个单元测试、13 个 Docker 集成测试、前端 34 个组件/契约测试、
16 个 Playwright E2E、Lint 与生产构建全部通过。

F4 第六批于 2026-09-07 完成：新增管理员只读 `/rag/settings` 运行快照，按环境变量、YAML、
默认值的真实覆盖结果汇总 RAG 参数；Provider 凭据只返回脱敏片段。Chat 三档、Embedding 和
Rerank 候选统一展示是否配置、是否启用及 CLOSED/OPEN/HALF_OPEN 熔断状态。前端新增
`/admin/settings` 运行登记册，以候选链、模型表、Provider 连接和检索参数构成高密度运维页面，
样式独立位于 runtime feature 目录。至此 F4 规划的六批管理面全部完成。

### M5 韧性与生产化

- [x] Redis 跨实例流任务取消
- [x] 公平排队限流与多实例一致性压测
- [x] PG Worker 多实例领取与租约故障恢复
- [x] 容量与性能基准
- [x] 提交/消费幂等保护
- [x] 评测接口与质量回归数据集

M5 第一批于 2026-09-08 完成：流任务注册时写入带 TTL 的 Redis 归属键；停止接口按当前用户
校验归属，写取消标记并发布广播。任务所在实例执行本地幂等取消，注册、模型取消动作绑定及
订阅重连均回放持久标记，覆盖早停和 Pub/Sub 断线窗口。Redis 协调设置 500ms 预算，异常时
不阻塞问答主链路并保留单实例取消。单元测试覆盖权限、幂等、跨实例、早到标记、漏广播回放
与 Redis 超时降级，Docker 集成测试使用真实 Redis 验证双实例广播。

M5 第二批于 2026-09-08 完成：问答入口在 `meta` 后进入 Redis FIFO 队列，Lua 原子完成
队头僵尸清理、permit 扣减与租约登记；排队超时按 `reject → finish(REJECTED) → done`
终结并落库。实例内 Redis 操作门闩吸收连接池突发，真实 Redis 测试覆盖双实例严格顺序、
租约回收、重复释放和 200 请求下的全局容量上限。

M5 第三批于 2026-09-09 完成：PG 队列 claim/fail/recover 增加同事务领域状态 hook，保证
`t_async_task`、知识文档和分块日志一起提交或回滚；两个恢复器并发扫描同一过期任务时，
仅一个实例完成重投或终态失败。Worker 续租明确丢失 owner 后会取消旧执行协程，避免接管后
继续产生副作用；租约、心跳、首次恢复延迟、恢复周期和空闲轮询均已配置化。真实 PostgreSQL
测试覆盖 8 Worker/60 任务无重复领取、claim 故障原子回滚、崩溃重投及超重试失败。

M5 第四批于 2026-09-09 完成：chat 按 userId 使用带唯一 token 的 Redis 提交锁，覆盖完整
SSE producer 生命周期，同用户并行请求返回稳定冲突文案；Worker 按 eventId 使用
FIRST_RUN/CONSUMING/CONSUMED 三态消费标记。业务异常删除标记允许重试，业务已完成但 PG
确认前崩溃时，接管 Worker 跳过副作用并补做队列确认。Redis 消费标记故障时降级到 PG
owner、eventId 和领域唯一约束，不把 Redis 作为最终正确性前提；CONSUMING 冲突采用不增加
retryCount 的延迟归还，避免基础设施等待耗尽业务重试次数。

M5 第五批于 2026-09-09 完成：新增不访问模型供应商的可重复容量脚本，覆盖 Redis FIFO、
临时 PG 多 Worker 队列和内存 SSE。压测发现并修复等待请求各自轮询造成的 Redis 风暴，改为
每 API 实例单 dispatcher + Pub/Sub 唤醒 + 定频兜底；1000 请求本机吞吐由 54.66 提升至
1443.57 op/s，严格 FIFO、全局不超发、无 permit 泄漏的真实 Redis 测试保持通过。详细参数
与结果见 `05-流量保护与韧性设计.md` 第 11 节。

M5 第六批于 2026-09-10 完成：按 `eval.enabled` 条件注册 `/rag/eval`，复用线上改写、意图、
作用域、检索与 MCP 组件，但不执行最终回答生成；文档指标通过 Chunk→内部文档→去扩展名的
业务文档 ID 两跳映射计算。仓库新增可直接导入的三份基准语料、30 条版本化黄金问题和批量质量
门槛脚本，覆盖 Hit Rate、Recall、MRR、Context Precision、Intent Accuracy 与 P95。
`/admin/eval` 提供单题质量实验台，证据、路由和意图在同页判读，样式独立于 global CSS；
桌面与 390px 浏览器验收通过。当前自动化结果为后端 202 项单元测试、24 项 Docker 隔离
集成测试、前端 36 项单元/契约测试、17 项 Playwright E2E，以及 Ruff、Lint、类型检查和
生产构建全部通过。至此 M5 六项规划全部完成。

2026-09-10 首次真实质量基线使用独立知识库和 6 条黄金问题，初始结果为
Hit Rate、Doc Recall、MRR 均 1.0，Context Precision 0.2，Latency P95 3496ms。随后将百炼
`relevance_score` 回写至检索切片，并以 `ai.rerank.min_score=0.30` 过滤低相关上下文；
同集复测的 Context Precision 提升到 0.8333，其余三项仍为 1.0，P95 3639ms，
6/6 无错误并通过 Context Precision ≥ 0.75 的新门槛。

2026-09-11 黄金集扩展为 30 题，增加标准答案、关键事实词和严格知识库隔离。
30 题分数扫描后将 `ai.rerank.min_score` 校准为 0.40；真实复测 Hit Rate、Doc Recall、
MRR 均为 1.0，Context Precision 0.9556，Latency P95 3401ms，30/30 无错误通过门槛。
后续答案模式直接复用线上 KB Prompt 和 Chat 模型路由，不创建会话或落库消息；
页面通过显式开关启用，同页展示生成答案与独立耗时。批量脚本以归一化的原子事实词和
`|` 同义表达计算关键事实覆盖率与完整答案率。
真实 30 题答案回归的 Keyword Recall 和 Complete Rate 均为 1.0，Answer P95 2263ms；
同轮检索 Context Precision 0.9556、Retrieval P95 3144ms，30/30 通过全部门槛。

2026-09-12 黄金集扩展为 v3 的 42 题，新增 12 个统一标记 `hard` 的跨文档、精确边界、否定条件、
时间线归纳和相似数字消歧用例。批量器支持重复 `--tag` 并按全部标签匹配，可独立验证困难集，
避免每次校准都重复调用基础 30 题。困难集真实语义回归 12/12 通过：Hit/Recall/MRR、答案事实覆盖、
答案完整率、Semantic Score 与 Semantic Pass Rate 均为 1.0，Context Precision 0.9722，
Retrieval P95 3464ms，Answer P95 4587ms；批次已保存为 `hard-semantic-12`。

管理端评测账本支持识别无答案专项报告，并将四格详情切换为无证据率、正确拒答率、语义正确性和
回答 P95；普通事实问答报告保持原指标布局，样式仍完全复用 `EvalPage.css`。无答案集最终 8/8
正确拒答且语义裁判 8/8 通过，报告保存为 `unanswerable-final-8-v2`。

2026-09-12 增加评测批次档案：批量脚本可显式发布摘要与逐题结果，管理页在单题实验台之外增加
工程账本式历史区，展示通过状态、命中率、Context Precision、P95 和相邻批次变化；选择批次后
展开答案事实覆盖、完整答案率及失败用例。样式继续限定在 `features/eval/EvalPage.css`，不向
`globals.css` 集中。

同日增加可选语义裁判展示。批次详情用同一指标网格补充 Semantic Score，不新增独立大卡片；
逐题列表将裁判 `PARTIAL/FAIL` 与门槛失败合并为风险项，展示裁判原因，保持管理者定位问题的
单一路径。语义裁判默认只观察，避免把同源模型偏好包装成确定性质量结论。
...
- 三个 Dashboard 接口均要求管理员权限，非法窗口回落默认值，时间点输出 epoch milliseconds。
- 使用量、延迟和质量指标与 `07` 文档口径一致，数据库无数据时返回稳定的零值和补零序列。
- `/admin` 默认进入系统概览，周期/指标切换可用，桌面和 390px 小屏没有横向页面溢出。

### F4 用户与审计准出

- 默认管理员不可修改或删除；非法角色、重复用户名、错误当前密码返回稳定业务错误。
- 用户角色、身份和密码变化后全部旧 token 立即失效，用户表和审计快照均不暴露密码哈希。
- 审计列表筛选写入 URL，详情可查看前后快照与字段 Diff；桌面和 390px 小屏可完成查询。

### F4 Pipeline 与任务准出

- Pipeline 名称唯一；节点图的重复 ID、悬空引用、环、多起点和不可达分支由服务端拒绝。
- 同步调试始终落任务终态和逐节点日志；条件跳过、失败中断、耗时、错误与摘要输出可回查。
- `t_async_task` 只读页保持 PG 队列原始 `pending/running/success/failed` 状态，不伪装为 Pipeline 运行。
- Pipeline 与运行时间字段统一为 epoch milliseconds，凭证、源文件字节和向量不进入响应或日志。

### F4 智能体与 Prompt 准出

- 7 个槽位来自同一权威枚举，WORKFLOW/AGENT 的有效槽位总数分别稳定为 6/4，未知槽位与必需占位符缺失返回明确业务错误。
- 内置智能体不可编辑、删除或保存槽位；激活保持全局单条，激活项不可直接删除，空白 Prompt 恢复到内置默认。
- 闲聊、知识库、MCP 和推荐问题运行路径使用 resolver；写操作清除 Redis 缓存，缓存异常透明回源 PostgreSQL。
- `/admin/agents` 仅管理员可访问，桌面与 390px 可完成选择、激活和 Prompt 编辑；功能样式不进入 `globals.css`。

### F4 MCP 管理与调试准出

- MCP Server 不可达时 API 正常启动；恢复后可由管理员重新发现，工具按限定 key 注册且同名不冲突。
- Server/Tool 启停覆盖写入 PostgreSQL，重启后保持；所有六条管理路由仅管理员可访问，写操作和调试可审计。
- 显式调试不调用 LLM，运行时意图调用才执行 Schema 驱动提参；缺必填项追问，非法参数和协议异常不抛断问答主链路。
- `/admin/mcp` 在桌面与 390px 均可完成状态查看、工具选择和调试；功能样式不进入 `globals.css`。

### F4 运行时与模型设置准出

- `/rag/settings` 仅管理员可访问，不返回完整 API Key；短密钥完全遮盖，长密钥仅保留前 6 后 4。
- 三个 Chat 档位候选顺序与当前配置一致，Embedding/Rerank 默认模型准确；未配置、停用、熔断和探测状态不能误写为健康。
- 页面只读，明确配置覆盖顺序与重启语义；桌面和 390px 可检查全部区域，功能样式不进入 `globals.css`。
