...
桌面与 390px 浏览器验收通过。当前自动化结果为后端 190 项单元测试、23 项 Docker 隔离
集成测试、前端 36 项单元/契约测试、17 项 Playwright E2E，以及 Ruff、Lint、类型检查和
生产构建全部通过。至此 M5 六项规划全部完成。

2026-09-10 首次真实质量基线使用独立知识库和 6 条黄金问题，结果为 Hit Rate、Doc Recall、
MRR 均 1.0，Context Precision 0.2，Latency P95 3496ms，零接口错误并通过质量门槛。
正确文档均位列第一；`topK=10` 引入旁支 Chunk 的噪声已作为下一阶段检索精度优化基线记录。
