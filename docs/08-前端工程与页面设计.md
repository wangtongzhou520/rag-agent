...
| `/admin/knowledge` | 知识库 | admin | READY |
| `/admin/knowledge/:kbId` | 文档 | admin | READY |
| `/admin/knowledge/:kbId/docs/:docId` | Chunk | admin | READY |
...
视觉语气为「**清晰、精密、有流动感的蓝色智能工作台**」。页面的记忆点不是渐变大标题，而是一条连接问答与 Trace 的「RAG 信号轨」：
...
- 导航可使用 `blue-900 → blue-800` 的轻微纵向层次，但不使用近黑的深墨色。
...
- 点击节点在右侧 Drawer 展开 `extraData`，支持 JSON 折叠和复制；`extraData` 是对象，不按字符串二次 parse。
...
- 时间按 ISO 字符串传输，格式化时显式使用用户时区。
- 前端契约 fixture 从后端测试样本导出，不依赖人工长期双维护。

## 13. 权限与安全

- `/chat*` 和 `/preview*` 需登录；`/admin*` 需 admin。
- **当前实现差距**：知识库、意图树、映射和 Trace 路由目前主要挂 `require_user`，前端可先联调，但生产上线前后端必须统一补 `require_admin`；不得用前端隐藏菜单代替服务端授权。
...
- [ ] 创建 `frontend/` 工程、strict TypeScript、alias 和命令
- [ ] 建立蓝色 Token、字体、Button/Input/Dialog/Table 基线
- [ ] 实现 API client、Result 解包、ApiError、本地 proxy
- [ ] 实现 Router、RequireAuth、RequireAdmin、ErrorBoundary
- [ ] 建立 Vitest/MSW/Playwright 骨架
...
- [ ] 问答首页、输入、深度思考开关
- [ ] SSE 六事件解析和状态机
- [ ] Markdown、thinking、sources、引用定位
- [ ] 文档预览
- [ ] 断线、空检索、歧义和中断验收
...
- [ ] 查询词映射 CRUD
- [ ] Trace 列表与信号轨详情
- [ ] 管理 API 统一补齐后端 `require_admin` 生产守卫
