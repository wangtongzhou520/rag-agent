...
- 请求体（OpenAI 兼容）：`{"model": ..., "input": [...], "dimensions": <candidate.dimension>, "encoding_format": "float"}`；子类钩子 `max_batch_size()`（默认 0 不限）超限时分片并保持返回顺序与输入一致。百炼 `qwen3.7-text-embedding` 按官方同步接口上限固定为每批 20 条。
- 非 200 响应仅提取服务端错误码和消息（最多 500 字符）进入任务日志，不记录请求头、密钥或完整响应。
...
```
  ...
  响应 `results[]` 提供原文档 `index` 和 `relevance_score`；客户端校验索引和分数，将分数回写到 `RetrievedChunk.score`，按结果顺序重排并过滤 `relevance_score < ai.rerank.min_score`。默认阈值 `0.40` 由 M5 30 题黄金集校准。
- `NoopRerankClient`（provider=`noop`，注册表中 `priority: 100` 兜底）：不重排、不套用分数阈值，`topN<=0 || size<=topN` 返回原列表，否则截断前 topN。百炼熔断/失败时经 fallback 自动落到 noop，检索链路永不因 rerank 中断。
...
- **任何 agents/prompt 写操作（增删改、激活、保存槽位）后必须 `DELETE ragent:agent:resolved-prompts`**（`AgentPromptCache.clear`），否则改动最长 1h 后才生效。
