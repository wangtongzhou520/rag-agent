...
- `BaiLianRerankClient`（百炼 dashscope 私有协议，非 OpenAI 兼容）：
  1. 先按 chunk id 去重（保序）；
  2. `topN <= 0` 或去重后 `size <= topN` → 直接返回，不发请求；
  3. 请求体：

```json
{
  "model": "qwen3-rerank",
  "input": {"query": "...", "documents": ["chunk文本", ...]},
  "parameters": {"top_n": 10, "return_documents": true}
}
```

  4. 响应 `output.results[]`：`index`（原文档下标，越界丢弃）+ `relevance_score`；按结果顺序重排，`relevance_score` 写回 chunk 的 score；按 index 去重，只保留 results 命中的候选。
