...
| # | 能力 | 实现 | 验收要求 |
|---|---|---|---|
| F1 | SSE 问答全局并发公平排队 | `fair_limiter.py` + `semaphore.py` | Redis Key、Lua、超时清理和参数默认值稳定 |
...
| F5 | Lua 原子 claim + permit 获取 | `fair_limiter.py` 内嵌 Lua | 清理队头僵尸，并在同一原子操作中扣减 permit |
| F6 | Redis 连接池突发保护 | 实例内有界异步门闩 | 入队、轮询和释放最多 16 个并发 Redis 操作 |
| F7 | 定频轮询 + pub/sub 批量唤醒 | asyncio `Event` + Redis pub/sub | Event 天然合并通知；轮询保证断线时语义不退化 |
| F8 | 等待协程 GRANTED/TIMED_OUT/CANCELLED | deadline + `finally` 幂等 cleanup | 获得许可后由业务 `finally` 唯一释放 |
...
flowchart TD
    ...
    G --> J[Lua claimAndAcquire]
    J --> K{队头且有 permit?}
    K -- 否 --> I[定频轮询 + Pub/Sub 唤醒]
    K -- 是 --> P[原子出队、删 entry、扣 permit、写租约]
...
2. **单原子协调点**：队头检查、过期租约回收、出队、扣 permit 与新租约登记在同一 Lua 中完成。
3. **grant 顺序**：只有第一个存活队头可以成功，Redis 串行执行 Lua，跨实例仍严格按 score 发放。
4. **GRANTED 后 cancel 不提前释放 permit**：permit 生命周期由业务 `try/finally` 接管，防止同一 slot 同时让给两个请求。
5. **cleanup 幂等**：等待协程的 `finally` 总会 ZREM 队列并删除 entry；有实际变更才发布通知。
6. **连接池背压**：每实例最多 16 个并发 Redis 操作，突发请求不会在入队前耗尽连接池。

### 4.1.1 公平排队限流概念类图

下图保留领域概念的拆分视角。当前 Python 实现为减少跨对象状态竞争，将 `ChatQueueLimiter`
作为 `FairDistributedRateLimiter` 的语义化子类，并把 FairQueue、通知合并器和 claim 脚本内聚在
`fair_limiter.py`；`PermitExpirableSemaphore` 单独位于 `semaphore.py`。
...
- `RAGChatService` 负责入口编排和 §4.2 reject 流程，`ChatQueueLimiter` 只暴露 acquire/release。
- 等待状态由单个 asyncio 协程及 deadline 表达；取消、超时都通过同一个 `finally` 清理入口收敛。
- claim 脚本内嵌在 `fair_limiter.py`，返回 permitId 或 nil，与 §5.1 伪代码一一对应。
...
### 5.1 Lua 原子 claim + acquire（`fair_limiter.py`）

```lua
-- KEYS: queue/counter/permit-zset/notify；ARGV: entryPrefix/requestId/now/leaseUntil/permitId
local slack = 16
local head = ZRANGE queueKey 0 slack
local firstLive = nil
for member in head:
    if EXISTS(entryPrefix .. member): firstLive = member; break
    else:
        ZREM queueKey member
if firstLive != requestId: return nil
回收 permit-zset 中已到期租约并恢复 counter
if counter <= 0: return nil
ZREM queueKey requestId; DEL entryPrefix .. requestId
DECR counter; ZADD permit-zset leaseUntil permitId
PUBLISH notify permit_changed
return permitId
```

- claim、扣减计数与写租约在一段 Lua 内完成，不存在“已出队但未取得 permit”的中间态。
- `slack=16`、entry TTL 缓冲 `5000ms` 为硬编码常量，不做配置。
- 每个 API 实例用 16 槽异步门闩约束 Redis 瞬时操作数；全局公平性仍由 Redis 队列决定。
- 每个实例只有一个 dispatcher 协程参与队头 claim；本地等待请求按 Redis score 排序并由
  Future 接收发放结果。Pub/Sub 负责即时唤醒，50ms 定频检查兜底，Redis 空闲轮询量从
  `O(本机等待请求数)` 降为 `O(API 实例数)`。

### 5.2 单实例 dispatcher 通知合并（F7）

```
acquire(requestId):
    score = Redis INCR + ZADD
    pending[requestId] = (score, deadline, future)
    dispatchEvent.set()
    return await future until deadline

dispatcher():
    wait dispatchEvent or 50ms fallback
    localHead = min(pending, by=Redis score)
    permit = Lua claim(localHead)                         # 仅全局活队头成功
    if permit: future.set_result(permit); continue        # 继续填充可用容量
    else: wait next notification                          # 不让每个等待者独立轮询
```

两个实例都只尝试各自本地最早 score；不是全局队头的实例会失败并等待下一次通知，因此保持
严格 FIFO。超时、取消与 grant 竞争时，future 所有权检查会归还孤儿 permit，避免许可泄漏。
...
| 提交幂等 TTL | `rag.idempotency.submit-ttl-seconds` | `330s` | 必须大于 300s SSE 全局超时 |
| 消费幂等 TTL | `rag.idempotency.consume-ttl-seconds` | `3600s` | 必须大于任务租约 + 恢复扫描周期 |
...
| 任务租约 | `rag.task.lease-seconds` | `300` | 长于心跳间隔；过期后 task/document/log 原子恢复 |
| 任务心跳 | `rag.task.heartbeat-seconds` | `60` | 必须小于租约；明确失去 owner 时取消旧执行协程 |
| 恢复首次延迟 | `rag.task.recovery-initial-delay-seconds` | `30` | Worker 启动后第一次过期租约扫描 |
| 恢复扫描周期 | `rag.task.recovery-interval-seconds` | `60` | 多 Worker 通过 SKIP LOCKED 并发安全扫描 |
| Worker 空闲轮询 | `rag.task.poll-interval-seconds` | `1` | PG NOTIFY 丢失时的兜底轮询 |
...
```
提交幂等：
  chat key → idempotent-submit:key:{userId}
  获取锁失败 → ClientException(message)  # chat: "当前会话处理中，请稍后再发起新的对话"

消费幂等（Lua 一次往返）：
  GET + 不存在时 SET idempotent-consume:{eventId} "0:{token}" PX {ttl*1000}
...
Lua 在 Redis 服务端原子执行 GET/SET，因此没有两个 Worker 同时得到 FIRST_RUN 的窗口；
完成和回滚也校验唯一 token，旧 Worker 不能覆盖或删除接管者的标记。
CONSUMING 的 TTL 取 `任务租约 + 恢复周期 + 30s`，CONSUMED 的 TTL 使用配置值 3600s；
接管者遇到 CONSUMING 时通过 `TaskQueue.defer` 延迟归还，不增加业务 `retry_count`。

### 5.4.1 幂等框架类图

下图保留原始框架设计视角。当前实现将 Redis 操作收敛到两个执行器；chat 因 SSE 生命周期
不能使用普通路由装饰器，由路由显式持有提交锁，Worker 则显式编排消费三态。
...
- `SubmitLockExecutor` 在 SSE producer 创建前加锁，producer 的 `finally` 才释放；普通路由装饰器会在返回 `StreamingResponse` 时过早释放。
- `ConsumeMarkExecutor.try_begin` 返回三态：`FIRST_RUN` 放行、`CONSUMING` 延迟重试、`CONSUMED` 跳过业务并直接确认 PG 任务；成功置 `"1"`，异常 DEL 放行重试。
- 消费业务键固定使用 `event_id`；同一队列行被新事件覆盖时，新 event_id 不受旧事件完成标记影响。
...
│   └── fair_limiter.py     # FairDistributedRateLimiter / ChatQueueLimiter
│                           #   ├─ acquire(request_id) / release(permit) / start() / close()
│                           #   ├─ Event 合并 Pub/Sub 唤醒 + 200ms 定频轮询
│                           #   └─ 内嵌 claim-and-acquire Lua + Redis 操作门闩
├── framework/
...
- `acquire(request_id)` 在当前请求协程等待；返回 `Permit` 表示获准，返回 `None` 表示超时。
- 问答主链路在当前事件循环执行，不引入线程池；许可由 `RAGChatService` 的 `finally` 释放。
...
| 心跳续约 | 长任务 RUNNING 期间 worker 心跳续约 `lease_until`；owner 已丢失时立即取消旧 handler，临时数据库故障则重试到原租约截止 |
| 重复消费 | `event_id` 唯一、活跃任务部分唯一索引、业务副作用唯一约束/CAS；Redis 幂等仅作减压优化 |
| 重试 | 业务异常增加 `retry_count` 并指数退避；CONSUMING 冲突仅 defer、不计失败；超限置 `FAILED` |
...
- claim、fail、recover 通过事务 hook 同步推进领域状态；hook 异常会让整笔事务回滚，不留下半提交状态。
...
| 队头暂时无 permit | 保持原 score 留在队头，等待定频轮询或 Pub/Sub 唤醒 |
...
1. **Lua claim 测试**（真实 Docker Redis）：
   - 僵尸条目在队头窗口内被 ZREM，存活条目 rank 正确前移；
   - slack=16 边界：窗口外僵尸不清理、窗口外存活不晋级；
   - 只有第一个存活队头可以获得 permit；
   - entry 标记缺失即僵尸；claim 成功后标记被 DEL。
2. **信号量**：trySetPermits 幂等（二次调用不重置）；租约到期自动回收；重复 release 不透支计数。
3. **公平性**：并发 200 请求，断言跨实例不超发；permit=1 时 grant 顺序与入队 score 严格一致。
4. **等待终态**：grant 与 cancel/timeout 竞争时，队列清理幂等且 permit 不泄漏；GRANTED 后只由业务 finally 释放。
5. **通知降级**：Pub/Sub 断开时，定频轮询仍能继续发放许可。
...
## 11. M5 本地容量基准（Apple Silicon）

基准命令不调用模型供应商；Redis 使用随机前缀，PG 使用自动创建并删除的临时数据库：

```bash
RAGENT_DATASOURCE__PASSWORD='<local-password>' uv run python -m scripts.benchmark_m5
RAGENT_DATASOURCE__PASSWORD='<local-password>' uv run python -m scripts.benchmark_m5 \
  --redis-requests 1000 --redis-permits 50 --redis-work-ms 20 \
  --pg-tasks 1000 --pg-workers 16 --sse-messages 100000
```

2026-09-09 开发机（Darwin 25.5.0 arm64、Python 3.13.0、Docker Desktop
Redis/PostgreSQL）实测：

| 场景 | 操作数 | 吞吐(op/s) | P50(ms) | P95(ms) | P99(ms) | 峰值并发 | 拒绝 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Redis FIFO（优化后压力参数） | 1000 | 1443.57 | 449.908 | 645.120 | 662.437 | 50/50 | 0 |
| PG 任务队列（16 Worker） | 1000 | 351.34 | 28.293 | 85.613 | 142.665 | 16 | 0 |
| 内存 SSE sender | 100000 | 483149.85 | 0.002 | 0.002 | 0.003 | - | 0 |

Redis FIFO 原实现由每个等待请求独立轮询，1000 请求耗时 18.29s、吞吐 54.66 op/s；改成
每实例单 dispatcher 后耗时 0.69s、吞吐 1443.57 op/s，提升约 26.4 倍。该数据是本地回归
基线，不是生产 SLA；模型首包和生成速度归入质量评测批次。
