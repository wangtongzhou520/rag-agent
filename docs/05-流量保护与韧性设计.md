...
| 接口 | 方法 | 说明 |
|---|---|---|
| `/api/ragent/rag/v3/chat` | GET，`text/event-stream;charset=UTF-8` | 参数 `question`（必填）、`conversationId`（可选）、`deepThinking`（默认 false）。提交幂等 key=userId，冲突报 `当前会话处理中，请稍后再发起新的对话` |
| `/api/ragent/rag/v3/stop` | POST | 参数 `taskId`；`Result<Void>` 包装；默认提交幂等（path+uid+md5），文案 `您操作太快，请稍后再试` |

SSE 六事件（与前端约定，不可变）：`meta` / `message` / `finish` / `cancel` / `reject` / `done`。
