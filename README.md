...
| [09-CI质量门禁](docs/09-CI质量门禁.md) | 后端双平台、PG/Redis 集成、前端 E2E 与真实 RAG 评测门禁（本地执行；远端 Actions 未启用） | 本地已落地 |
...
| [12-育儿语料人工验收执行方案](docs/12-育儿语料人工验收执行方案.md) | 真实语料预检结论、导入步骤、题库与拒答题、边界取证、通过标准 | 已交付 |
| [13-未完成能力实施计划](docs/13-未完成能力实施计划.md) | 待办批次（定时刷新、Pipeline 线上接入、分块体系、意图配置、大文件、检索通道）、DoD 与开工清单 | 待执行 |
| [14-前端去AI化与视觉改版方案](docs/14-前端去AI化与视觉改版方案.md) | 去 AI 化量化标准、现状诊断、纸墨档案设计令牌、逐页与逐组件规范、实施批次与验收 | 待执行 |
...
向量通道，混合检索的三条增强通道仍在规划中。生产化部分已补齐全栈容器编排与四类质量门禁
（后端双平台、PG/Redis 集成、前端 E2E、容器构建；在本地执行，远端未启用 GitHub Actions）
与真实质量工作流；容器化部署的验收数据
